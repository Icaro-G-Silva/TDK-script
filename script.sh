...
cloning_repos() {
  ...
  _key=$(find /home -name id_rsa.pub)
  if [[ -f $_key ]]; then
    sudo -H -u $_username bash -c "mkdir dextra-repos && chmod 751 dextra-repos"
    cd dextra-repos
    if ! sudo -H -u $_username bash -c "git clone git@github.com:dextra/tsunami.git"
    then
      echo "Não foi possível clonar o repositório do tsunami"
    else
      echo "Repositório do tsunami clonado"
    fi
    if ! sudo -H -u $_username bash -c "git clone git@github.com:dextra/my-way.git"
    then
      echo "Não foi possível clonar o repositório do wavez"
    else
      echo "Repositório do wavez clonado"
    fi
  else
    echo "Certifique-se de ter configurado a chave SSH corretamente"
  fi
}
